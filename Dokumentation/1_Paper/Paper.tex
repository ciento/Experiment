...
\section*{Abstract}
	Autor: Berna, Überarbeitung: Jana\\
...
%Visuelle und auditive Zeitgeber verändern das Zeitempfinden einer Person innerhalb der VR, sofern diese passiv bleibt, keine Aufgaben lösen muss, etc etc
...
%Wallisch beschreibt, \glqq dass das menschliche Zeitempfinden, insbesondere die Empfindung von Dauer von allen möglichen Faktoren abhängig ist\grqq . \cite{Wallisch2003} S. 27 \\
...
%die Versuchspersonen die Zeit überschätzten, wenn sie passiv blieben. Die Sonnenbewegungen hatten einen großen Einfluss auf ihre Zeitwahrnehmung. Waren sie hingegen mit einer Aufgabe beschäftigt, unterschätzten sie die in ver VR vergangene Zeit und die Sonnenzyklen hatten keinen Einfluss. \cite{DeviceSystems2016}
...
Nach Unterschreiben der Einverständniserklärung wird der erste von insgesamt sechs Zeiteinschätzungstests durchgeführt, in welchem die Versuchsperson 40 Sekunden ohne Gespräch abzuschätzen hat. Die Versuchsperson gibt auf Fragebogen A alle für das Experiment relevanten internen Einflussfaktoren (vgl. Kapitel 1.3  Forschungsstand) an.
 %d.h. ob sie unter Drogen- oder Medikamenteneinflüssen steht, wie müde sie ist, ob sie an Gehirnschäden oder psychischen Erkrankungen leidet ihr Muttersprache.
...
Die ersten drei Zeiteinschätzungen werden in Relation zu den Einflussfaktoren Alter, Intelligenz, Körpertemperatur und Müdigkeit der Versuchspersonen gestellt. Die Streudiagramme sind gruppiert nach den Einschätzungen, die Versuchspersonen entweder \textit{ohne Gespräch} (Abb. \ref{img:alter30ohne} bis Abb. \ref{img:mued40ohne}) oder \textit{mit Gespräch} (Abb. \ref{img:alter40mit} bis Abb. \ref{img:mued40mit}) absolviert haben.


%%%%%%%%%%%%%%%% ALTER

\textbf{Zeiteinschätzungen ohne Gespräch}
\begin{figure}[H]
	\centering
...
\textbf{Zeiteinschätzungen mit Gespräch}
\begin{figure}[H]
	\centering
...
Abbildung \ref{SzenarienKurzLang} zeigt die Anzahl der Versuchspersonen, die die genannten Szenarien von kurz nach lang sortiert haben. D.h. konkret, dass z.B. insgesamt sieben Versuchspersonen \textbf{b} für das kürzeste Szenario hielten und dann aufsteigend über \textbf{a} und \textbf{d} nach  \textbf{c} sortiert haben. \\
...
Eine eindeutige Mehrheit gibt es hier nicht, allerdings wurden Szenario \textbf{a} mit 16 und Szenario \textbf{b} mit 15 Stimmen am meisten gewählt. 
...
Auch hier gibt es kein klare Mehrheit, dennoch heben sich mit 17 Stimmen Szenario \textbf{c} und mit 14 Stimmen Szenario \textbf{b} etwas ab. 
Trotz dass es keine einheitliche Meinung gibt, welches Szenario das längste oder kürzeste ist, zeichnen sich ein paar Tendenzen ab. So ist sich die relative Mehrheit der Versuchspersonen sicher, \textbf{a} sei das längste Szenario und dementsprechend Wenige behaupten das direkte Gegenteil. \textbf{was} Ähnlich verhält es sich bei Szenario \textbf{c}, welches auch verhältnismäßig oft als das längste bewertet wurde und nur sehr wenig als das kürzeste.\\
...
%Ein Faktor der die Zeitwahrnehmung beeinflusst haben könnte, ist hinsichtlich des Forschungspapiers "'Kinder, wie die Zeit vergeht!"' \textit{Über Paradoxien in der Zeitwahrnehmung von Katja Irle} \cite{Irle2017} der Spaß am Geschehen.
Spaß als möglicher Einflussfaktor wird separat betrachtet.
