...
Dazu wurden an Versuchspersonen, unter Berücksichtigung verschiedener Störfaktoren \textbf{wie z.B. das Geschlecht oder Alter,} Zeiteinschätzungstests durchgeführt, die später als Vergleichswerte dienen.
...
Diese Arbeit bezieht sich auf \textbf{drei oder zwei	} Studien zur Zeitwahrnehmung und -manipulation in der VR und berücksichtigt die bereits erforschten Erkenntnisse zu menschlicher Zeitwahrnehmung. \\
...
Die Testumgebung besteht aus vier Szenarien, wobei jedes Szenario aus einer Ampelkreuzung und einem Zeitgebern besteht. Es werden zwei visuelle und zwei auditive Zeitgeber verwendet, wobei davon jeweils einer schnell und einer langsam ist.
Um eine Beeinflussung durch eine festgelegte Reihenfolge der Szenarien auszuschließen, werden in alle möglichen Szenarienreihenfolgen von gleichmäßig vielen Versuchspersonen durchlaufen.
...
Abb. 1 zeigt die Testumgebung von oben. Der Aufbau der Welt ist statisch, es gibt einen Startpunkt und einen zu erreichenden Endpunkt, die Laufrichtung ist entgegen des Uhrzeigersinns vorgegeben. Vor jeder Abbiegung in Laufrichtung befindet sich ein Teleporter (roter Punkt), der durch eine Triggerbox aktiviert wird und die Versuchsperson an die gewünschte Stelle der Karte befördert. Durch die immer gleichaussehenden Ecken der Karte wird
 \textbf{der flow} der Versuchsperson nicht behindert. Alle Teleporter können beliebig miteinander verbunden werden, sodass  Auf diese Weise kann eine mögliche Beeinflussung der Probanden durch eine festgelegte Reihenfolge verhindert werden.
 Das blaue X symbolisiert die möglichen Startpunkte.
 Welcher tatsächlich gewählt wird ist von der verwendeten Reihenfolge der Szenarien abhängig. 
...
Im zweiten Raum wird das VR-Experiment durchgeführt, daher befinden sich in diesem Raum der Computer (et etc wie genau?) ocolus etc
