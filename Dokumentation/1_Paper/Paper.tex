...
\tikzset{lines/.style={draw=white},}
\pie[pos={8,0},radius = 2, color={greenOfApproval!60, redOfDisapproval!40},sum=auto, after number=,text=legend,every only number node/.style={text=black},style={lines}]{25/d kürzer als c,22/c kürzer als d}
\end{tikzpicture}
...
Zu Abb. \ref{SzenarioGut}-\ref{SzenarioSchlecht}: Beim Vergleich der beiden Diagramme, die die Probandenbefragung nach dem besten (Abb. \ref{SzenarioGut}) und schlechtesten Szenario (Abb. \ref{SzenarioGut}) darstellen, geht Szenario "'a"' (Zeitgeber: schnelles Ticken) als eindeutig am schlechtesten bewertetes hervor. Es wurde bei der Wahl des besten Szenarios auf den letzten und bei der Wahl des schlechtetsten Szenarios auf den ersten Platz gesetzt. Dagegen hebt sich das Szenario "'b"' (langsames Ticken) positiv hervor; es ist erstplatziert bei der Wahl des besten Szenarios und befindet sich bei der Wahl des schlechtesten auf dem vorletzten Platz. Dies ist gegenteilig wie erwartet, da zuvor vermutet wurde, dass schnelle Zeitgeber auch ein schnelleres Zeitempfinden erzeugen und dieses wiederum auch mit Spaßempfinden verbunden ist.\\
Der These nach, dass das beste Szenario - das, mit dem größten Spaßempfinden - auch dem am kürzesten empfunden entspricht, müsste die Reihenfolge der Abb. \ref{SzenarienKurzLang} (Sortierung der Szenarios von kurz nach lang) nun "'b c d a"' ergeben. Tatsächlich wurde "'a"' jedoch am häufigsten (17 von 47 Einschätzungen) an erster Stelle angegeben, "'b"' ist erst an zweiter Stelle zu finden (14 von 47 Einschätzungen).\\ 
